...
** Change in intensity over time can be computed in Excel by manipulating the data table or can be automated using a suitable programming language. An example analysis script written in R\supercite{CRAN:2017} is provided with this manuscript.
...
#+begin_src sh :tangle imagej-threshold-quant.sh :eval no :exports none
#! /bin/bash
...
data.thalf.stats <- dplyr::select(data.thalf.stats, -num, -min, -max, - stdev)
xtable::xtable(data.thalf.stats)
#+end_src

#+BEGIN_LaTeX
% latex table generated in R 3.4.0 by xtable 1.8-2 package
% Thu Jun 22 14:05:37 2017
\begin{table}[ht]
\centering
